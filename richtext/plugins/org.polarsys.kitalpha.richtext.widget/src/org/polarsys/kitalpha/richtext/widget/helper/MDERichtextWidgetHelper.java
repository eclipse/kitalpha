    ...
    if (activeWorkbenchWindow != null) {
      ...
      if (activePage != null) {
        return Arrays.stream(activePage.getEditorReferences())
            .filter(ref -> MDERichTextConstants.RICHTEXT_EDITOR_ID.equals(ref.getId())).map(ref -> ref.getEditor(false))
            .filter(Objects::nonNull).map(MDERichTextEditor.class::cast).collect(Collectors.toList());
      }
    }
