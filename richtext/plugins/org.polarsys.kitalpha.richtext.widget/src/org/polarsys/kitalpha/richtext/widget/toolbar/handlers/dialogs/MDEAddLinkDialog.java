 ...
 ******************************************************************************/
package org.polarsys.kitalpha.richtext.widget.toolbar.handlers.dialogs;
...
import org.polarsys.kitalpha.richtext.widget.toolbar.handlers.links.LinkManager;
...
public class MDEAddLinkDialog extends MDEOkCancelDialog {
	...
	private void createBrowserButton(Composite composite) {
		...
		browseButton.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent event) {
				...
				if (linkType.equalsIgnoreCase(LinkManager.FILE_LABEL)){
					path = linkManager.getPath(linkType, basePath);
				}
				if (linkType.equalsIgnoreCase(LinkManager.LOCAL_LABEL) || 
						linkType.equalsIgnoreCase(LinkManager.MODEL_ELEMENT_LABEL) ||
	...
	private void createListLinksType(Composite composite) {
		...
		linkTypeCombo.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent event) {
				...
				browseButton.setEnabled(!LinkManager.URL_LABEL.equals(linkType));
				urlLabel.setEnabled(LinkManager.URL_LABEL.equals(linkType) || LinkManager.FILE_LABEL.equals(linkType) || LinkManager.LOCAL_LABEL.equals(linkType));
				urlText.setEnabled(LinkManager.URL_LABEL.equals(linkType) ||  LinkManager.FILE_LABEL.equals(linkType) || LinkManager.LOCAL_LABEL.equals(linkType));
				if (LinkManager.URL_LABEL.equals(linkType)) {
	...
	protected void okPressed() {
		...
		if (url != null && url.length() > 0) {
			...
			richText.insertHTML(encodedURL);
		}
