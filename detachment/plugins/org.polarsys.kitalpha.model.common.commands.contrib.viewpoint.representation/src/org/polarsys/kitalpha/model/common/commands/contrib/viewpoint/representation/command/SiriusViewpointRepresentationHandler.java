...
public class SiriusViewpointRepresentationHandler {
	...
	private boolean deleteDiagram(final DDiagram diagram){
		...
		if (isInViewpoints(getAllSiriusViewpointsFromADViewpoints(), vp)){
			Collection<DRepresentation> allRepresentationDescriptors = DialectManager.INSTANCE.getAllRepresentations(session);
			for (DRepresentation repDesc : allRepresentationDescriptors) {
				if (repDesc.equals(diagramDescription)){
	 ...
	 * @param descriptor
	 */
	private void deleteRepresentations(final DRepresentation descriptor){
	...
	private void deleteRepresentations(final DDiagram diagram){
		final Collection<DRepresentation> descriptors = DialectManager.INSTANCE.getRepresentations(diagram, session);
		
		Command deleteRep = new RecordingCommand(session.getTransactionalEditingDomain(), "Delete Representations") { //$NON-NLS-1$
			
			@Override
			protected void doExecute() {
				for (DRepresentation descriptor : descriptors) {
					DialectManager.INSTANCE.deleteRepresentation(descriptor, session);
				}
			}
		};
