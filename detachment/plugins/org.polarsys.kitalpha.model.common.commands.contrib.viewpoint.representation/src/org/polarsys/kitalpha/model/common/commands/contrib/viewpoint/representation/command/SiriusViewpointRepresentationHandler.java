...
public class SiriusViewpointRepresentationHandler {
	...
	private void deleteDViews(){
		...
		if (usedSiriusViewpoints == null) {
			//NPE thrown when at unreferencing unknown references (sirius viewpoint does not exists
			// to initialize the list we try to get the viewpoints activiated on the session
			usedSiriusViewpoints = getAllSiriusViewpointsFromADViewpoints();
		}
