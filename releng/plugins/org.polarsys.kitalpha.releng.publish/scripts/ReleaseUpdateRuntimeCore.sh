...
# Component name
export COMPONENT_NAME="runtimecore"
# Update project name
export UPDATE_PRJ_NAME="org.polarsys.kitalpha.releng.runtime.core.updatesite"
...
if [ "$LOG" = "true" ]; then
	echo ">>> Run cmd 1 ReleaseUpdateRuntimeCore.sh start : . $BASEDIR/global-parameters.sh $BUILD_TYPE"
fi
# Extract global parameters (ie Publish path)
. $BASEDIR/global-parameters.sh $BUILD_TYPE
if [ "$LOG" = "true" ]; then
	echo ">>> Run cmd 1 ReleaseUpdateRuntimeCore.sh start : . $BASEDIR/release-parameters.sh $PRODUCT_NAME $COMPONENT_NAME $BUILD_TYPE"
fi
# Extract global parameters (ie VERSION)
. $BASEDIR/release-parameters.sh $PRODUCT_NAME $COMPONENT_NAME $BUILD_TYPE
if [ "$LOG" = "true" ]; then
	echo ">>> Run cmd 1 ReleaseUpdateRuntimeCore.sh end"
fi

# Run release publish for this component
if [ "$LOG" = "true" ]; then
	echo ">>> Run cmd 2 ReleaseUpdateRuntimeCore.sh start : . $BASEDIR/ReleaseUpdate.sh $COMPONENT_NAME $TARGET_MANIFEST_NAME $TARGET_MANIFEST_FOLDER $BUILD_TYPE $BUILD_TYPE_PREFIX $BUILD_VERSION"
fi
...
if [ "$LOG" = "true" ]; then
	echo ">>> Run cmd 2 ReleaseUpdateRuntimeCore.sh end"
fi
