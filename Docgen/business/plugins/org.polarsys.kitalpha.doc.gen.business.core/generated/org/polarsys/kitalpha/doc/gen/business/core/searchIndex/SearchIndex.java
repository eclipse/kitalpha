...
public class SearchIndex extends org.polarsys.kitalpha.doc.gen.business.core.doccontent.ElementDocContent {
  ...
  protected final String TEXT_2 = ";" + NL + "\t\tvar conceptsList = new Array(tabCount);" + NL
      + "\t\tvar currentSize = 0;" + NL + "\t\tvar addedConceptCount = 0;" + NL + "\t\tvar conceptIcons =  {};" + NL
  ...
  protected final String TEXT_13 = NL + NL + NL + "\t    $(\"#input1\").keyup(function () {" + NL
      ...
      + "\t      \t\tvar index = concepts[i][0].toLowerCase().indexOf(encodeURIComponent(value).toLowerCase());" + NL
      + "\t      \t\tif(index != -1) {" + NL + "\t\t      \t\tvar value2 = concepts[i][0];" + NL
      + "\t      \t\t\tvar value3 = concepts[i][1];" + NL + "\t      \t\t\tvar value4 = concepts[i][2]; //icon" + NL
  ...
  protected void method_content(final StringBuffer stringBuffer, final PatternContext ctx) throws Exception {
    ...
    for (Entry<String, IndexItem> entry : indexItems.entrySet()) {
      ...
      stringBuffer.append(
          EscapeChars.removeNewline(entry.getValue().getConceptName().replace("\\", "\\\\").replace("\"", "\\\"")));
