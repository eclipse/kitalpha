...
import org.polarsys.kitalpha.common.sirius.extension.rotativeimage.Activator;
import org.polarsys.kitalpha.common.sirius.extension.rotativeimage.figures.Rotative4ImagesSVGWorkspaceImageFigure;
import org.polarsys.kitalpha.common.sirius.extension.rotativeimage.figures.RotativeSVGWorkspaceImageFigure;
import org.polarsys.kitalpha.common.sirius.extension.rotativeimage.figures.RotativeWorkspaceImageFigure;

/**
 * Checks the displayed images in rotative ports. Compares displayed images with Image registers content. PNG images are
 * stored in the static {@link org.polarsys.kitalpha.common.sirius.extension.rotativeimage.Activator ImageRegistry} SVG
...
public class RotativeImageDisplayTest extends SiriusDiagramTestCase {
	...
	protected static final String ROTATIVEIMAGE_TEST_PLUGIN_NAME = "/org.polarsys.kitalpha.common.sirius.extension.rotativeimage.junit.tests";
	...
	private static final String ROTATIONICON = "org.polarsys.kitalpha.common.sirius.extension.rotativeimage.junit.tests/icons/rotationIcon";

	private static final String ROTATIONICON_SVG = ROTATIONICON + SVG;

	private static final String FOURIMAGESICON_PREFIX = "/org.polarsys.kitalpha.common.sirius.extension.rotativeimage.junit.tests/icons/4imagesIcon";
	...
	public void testPNGDiagramImages() {
		...
		for (Entry<String, Integer> entry : dDiagramElementToPositionConstantRotationPNG.entrySet()) {
			...
			WorkspaceImage wi = (WorkspaceImage) editPart.resolveSemanticElement();
			Image expectedImage = registry.get(wi.getWorkspacePath() + position);
		...
		for (Entry<String, Integer> entry : dDiagramElementToPositionConstantFourImagesPNG.entrySet()) {
			...
			// Get expected image
			Image expectedImage = registry.get(FOURIMAGESICON_PREFIX + PNG + position);
			compareImages(figure, expectedImage, PositionConstants.NORTH);
		}
	}
	...
	public void testFaultyRotativeDiagramImages() {
		...
		for (Entry<String, String> entry : dDiagramElementsToFileExtensionForFauly4Images.entrySet()) {
			...
			if (figure instanceof RotativeWorkspaceImageFigure) {
				// Get expected image
				ImageRegistry registry = Activator.getDefault().getImageRegistry();
				Image expectedImage = registry.get(FOURIMAGESICON_ERROR + PNG + PositionConstants.NORTH);
	 ...
	 */
	private void compareImages(RotativeWorkspaceImageFigure figure, Image expectedImage, int rotation) {
		...
		for (int x = 0; x < expectedImageData.width; x++) {
			for (int y = 0; y < expectedImageData.height; y++) {
				...
				assertEquals(expectedImageData.getPixel(expectedX, expectedY), actualImageData.getPixel(x, y));
				assertEquals(expectedImageData.getAlpha(expectedX, expectedY), actualImageData.getAlpha(x, y));
			}
		}
	}
