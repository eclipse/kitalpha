    	...
    	steps {
	    	withSonarQubeEnv('Sonar') {
				...
				sshagent ( ['projects-storage.eclipse.org-bot-ssh']) {
	          		sh '''
	          			mvn $SONAR_MAVEN_GOAL -PKitalphaSonar -f all/pom.xml -Dsonar.host.url=https://sonar.eclipse.org/ -Dsonar.projectKey=org.polarsys:org.polarsys.kitalpha -Dsonar.login=$SONAR_UN -Dsonar.password=$SONAR_PW
	          		'''
	          	}
	        }
        }
