...
public class VpspecContentProposalPriorities extends ContentProposalPriorities {
	...
	public void adjustKeywordPriority(ICompletionProposal proposal, String prefix) {
		...
		if (access instanceof VpspecGrammarAccess) {
			...
			if (!affected && proposal.getDisplayString().matches(grammar.getViewpointAccess().getDataKeyword_8_0().getValue())) {
				keywordPriority = dataPriority;
				affected = true;
			}
			if (!affected && proposal.getDisplayString().matches(grammar.getViewpointAccess().getTypeUIKeyword_9_0_0().getValue())) {
				keywordPriority = uiPriority;
				affected = true;
			}
			if (!affected && proposal.getDisplayString().matches(grammar.getViewpointAccess().getTypeDiagramsKeyword_10_0_0().getValue())) {
				keywordPriority = diagramPriority;
				affected = true;
			}
			if (!affected && proposal.getDisplayString().matches(grammar.getViewpointAccess().getTypeServicesKeyword_11_0_0().getValue())) {
				keywordPriority = serviceSetPriority;
				affected = true;
			}
			if (!affected && proposal.getDisplayString().matches(grammar.getViewpointAccess().getTypeBuildKeyword_12_0_0().getValue())) {
				keywordPriority = buildPriority;
				affected = true;
			}
			if (!affected && proposal.getDisplayString().matches(grammar.getViewpointAccess().getTypeConfigurationKeyword_13_0_0().getValue())) {
