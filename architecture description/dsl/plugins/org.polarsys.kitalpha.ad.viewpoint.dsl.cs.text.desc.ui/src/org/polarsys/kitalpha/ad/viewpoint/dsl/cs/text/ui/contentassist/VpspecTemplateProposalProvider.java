...
public class VpspecTemplateProposalProvider extends CommonTemplateProposalProvider {
			...
			for (Template template : templates) {
				...
				if (validate(template, templateContext)) {	
					...
					if (contextTypeId.matches(TemplateIDs.TEMPLATE_PREFIX + SEPARATOR + "kw_" + UI.class.getSimpleName()) ||
							contextTypeId.matches(TemplateIDs.TEMPLATE_PREFIX + SEPARATOR + "kw_" + access.getViewpointAccess().getTypeServicesKeyword_15_0_0().getValue()) ||
							contextTypeId.matches(TemplateIDs.TEMPLATE_PREFIX + SEPARATOR + "kw_" + access.getViewpointAccess().getTypeDiagramsKeyword_14_0_0().getValue()) ||
							contextTypeId.matches(TemplateIDs.TEMPLATE_PREFIX + SEPARATOR + "kw_" + access.getViewpointAccess().getTypeBuildKeyword_16_0_0().getValue()) ||
							contextTypeId.matches(TemplateIDs.TEMPLATE_PREFIX + SEPARATOR + "kw_" + access.getViewpointAccess().getTypeConfigurationKeyword_17_0_0().getValue())) {	
					...
					if (contextTypeId.matches(TemplateIDs.TEMPLATE_PREFIX + SEPARATOR + "kw_" + access.getViewpointAccess().getTypeDiagramsKeyword_14_0_0().getValue())) {						
						INode currentNode = context.getCurrentNode();
						INode nextSibling = currentNode.getNextSibling();
						if (nextSibling != null) {
							String text = nextSibling.getText();
	...
	protected String getTemplatePattern(String templateIdentifier, String shortName) {		
		VpspecGrammarAccess access = (VpspecGrammarAccess)grammar;		
		if (templateIdentifier.equals(TemplateIDs.NEW_BUILD_TEMPLATE)) {
			return access.getViewpointAccess().getTypeBuildKeyword_16_0_0().getValue() + " " + VARIABLE_NAME;
		}
		if (templateIdentifier.equals(TemplateIDs.NEW_CONFIGURATION_TEMPLATE)) {
			return access.getViewpointAccess().getTypeConfigurationKeyword_17_0_0().getValue() + " " + VARIABLE_NAME;
		}
		if (templateIdentifier.equals(TemplateIDs.NEW_DATA_TEMPLATE)) {
			return access.getViewpointAccess().getDataKeyword_12_0().getValue() + " "  + VARIABLE_NAME;
		}
		if (templateIdentifier.equals(TemplateIDs.NEW_DIAGRAM_TEMPLATE)) {
			return access.getViewpointAccess().getTypeDiagramsKeyword_14_0_0().getValue() + " "  + VARIABLE_NAME;
		}
		if (templateIdentifier.equals(TemplateIDs.NEW_SERVICES_TEMPLATE)) {
			return access.getViewpointAccess().getTypeServicesKeyword_15_0_0().getValue() + " "  + VARIABLE_NAME;
		}
		if (templateIdentifier.equals(TemplateIDs.NEW_UI_TEMPLATE)) {
			return access.getViewpointAccess().getTypeUIKeyword_13_0_0().getValue() + " "  + VARIABLE_NAME;
		}
		return null;
	}
