...
public class ReferencingViewpointElementJavaClass
		extends
		org.polarsys.kitalpha.ad.viewpoint.dsl.generation.explorer.contextual.common.AbstractAssociationJavaClass {
	protected static String nl;

	public static synchronized ReferencingViewpointElementJavaClass create(
	...
	protected final String TEXT_11 = NL + "\t\t\t\t\t\tresult.add(eObject);"
			+ NL + "\t\t\t\t}";
	...
	protected void method_computeReferencingClassesList(
			final StringBuffer stringBuffer, final PatternContext ctx)
			throws Exception {

		Data data = (Data) parameter.eContainer();
		for (org.polarsys.kitalpha.ad.viewpoint.dsl.as.model.vpdesc.Class clazz : data
				.getVP_Classes()) {
			for (AbstractAssociation abstractAssociation : clazz
				...
				if (abstractAssociation instanceof LocalClassAssociation
					...
					if ((localTarget != null && localTarget.equals(parameter))) {
						...
						addClass = ContextualExplorerCategoryCompliance
								.isDerivedClass(parameter, localTarget);
					}
		...
		new Node.DataLeaf(ictx.getNode(), getClass(),
				"computeReferencingClassesList", stringBuffer.toString());
	}
			...
			for (AbstractAssociation abstractAssociation : clazz
				...
				if (abstractAssociation instanceof LocalClassAssociation
					...
					if (vpAssociation.getLocalTarget().equals(parameter)
							|| ContextualExplorerCategoryCompliance
									.isDerivedClass(parameter,
											vpAssociation.getLocalTarget())) {
						String vpAssociationGetterName = ClassUtils.INSTANCE
								.getAssociationAccesssorName(vpAssociation);
						...
						if (ClassUtils
								.isAssociationMultiple(abstractAssociation)) {
	...
	protected void method_genSemanticImoprt(final StringBuffer stringBuffer,
		...
		if (referencingClassesList != null && referencingClassesList.size() > 0) {
			for (Object clazz_o : referencingClassesList) {
				...
				stringBuffer.append(ClassUtils.INSTANCE
						.getClassInterfaceFullyQualifiedName(clazz));
	...
	public boolean preCondition(PatternContext ctx) throws Exception {
		return ContextualExplorerCategoryCompliance
				.referencingViewpointCategoryCompliance(parameter);
	}
}