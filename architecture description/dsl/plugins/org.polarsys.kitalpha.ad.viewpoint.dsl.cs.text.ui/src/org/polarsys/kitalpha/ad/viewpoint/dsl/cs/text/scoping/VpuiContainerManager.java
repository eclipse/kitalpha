...
public class VpuiContainerManager extends StateBasedContainerManager {
	...
	protected List<IContainer> getVisibleContainers(List<String> handles, IResourceDescriptions resourceDescriptions) {
		...
		IContainer container = createVpuiContainer(handle, resourceDescriptions);
		result.add(container);
		return result;
	}
	
	protected IContainer createVpuiContainer(String handle, IResourceDescriptions resourceDescriptions) {
		IAllContainersState state = getState(resourceDescriptions);
		IContainerState containerState = new VpuiContainerState(handle, state);
		VpuiContainer result = new VpuiContainer(resourceDescriptions, containerState);
		return result;
	}
}
