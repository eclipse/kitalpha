...
public class getPropertyPattern extends org.eclipse.egf.emf.pattern.model.call.Class.ClassimplementedGenOperationTODOoverride {
	...
	protected final String TEXT_1 = "\t\tif (getPropertySet() != null) {" + NL + "\t\t\t// find Property inside the set" + NL + "\t\t\tfor (Property prop : getPropertySet().getNewProperties()) {" + NL
			+ "\t\t\t\tif (id.equals(prop.getId()))" + NL + "\t\t\t\t\treturn prop;" + NL + "\t\t\t}" + NL + "\t\t\t// if the Property is hidden return null" + NL
			+ "\t\t\tfor (Property prop : getPropertySet().getHiddenProperties()) {" + NL + "\t\t\t\tif (id.equals(prop.getId()))" + NL + "\t\t\t\t\treturn null;" + NL + "\t\t\t}" + NL + "\t\t}" + NL
			+ "\t\t// delegate to parents" + NL + "\t\tfor (Viewpoint vp : getParents()) {" + NL + "\t\t\tProperty prop = vp.getProperty(id);" + NL + "\t\t\tif (prop != null)" + NL
