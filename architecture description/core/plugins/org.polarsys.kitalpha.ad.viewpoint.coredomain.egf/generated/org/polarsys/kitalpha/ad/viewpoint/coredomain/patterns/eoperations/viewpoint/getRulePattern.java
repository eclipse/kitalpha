...
public class getRulePattern extends org.eclipse.egf.emf.pattern.model.call.Class.ClassimplementedGenOperationTODOoverride {
	...
	public static synchronized getRulePattern create(String lineSeparator) {
		nl = lineSeparator;
		getRulePattern result = new getRulePattern();
		nl = null;
		return result;
	}
	...
	protected final String TEXT_1 = "\t\tif (getRuleSet() != null) {" + NL + "\t\t\t// find rule inside the set" + NL + "\t\t\tfor (Rule rule : getRuleSet().getNewRules()) {" + NL
			+ "\t\t\t\tif (id.equals(rule.getId()))" + NL + "\t\t\t\t\treturn rule;" + NL + "\t\t\t}" + NL + "\t\t\t// if the rule is hidden return null" + NL
			+ "\t\t\tfor (Rule rule : getRuleSet().getHiddenRules()) {" + NL + "\t\t\t\tif (id.equals(rule.getId()))" + NL + "\t\t\t\t\treturn null;" + NL + "\t\t\t}" + NL + "\t\t}" + NL
			+ "\t\t// delegate to parents" + NL + "\t\tfor (Viewpoint vp : getParents()) {" + NL + "\t\t\tRule rule = vp.getRule(id);" + NL + "\t\t\tif (rule != null)" + NL + "\t\t\t\treturn rule;"
			+ NL + "\t\t}" + NL + "\t\treturn null;";

	protected final String TEXT_2 = NL;

	public getRulePattern() {
	...
	public boolean preCondition(PatternContext ctx) throws Exception {
		...
		return "getRule".equals(operation) && "Viewpoint".equals(cls);
