...
public class getRuleSetPattern extends org.eclipse.egf.emf.pattern.model.call.Class.ClassimplementedGenOperationTODOoverride {
	protected static String nl;

	public static synchronized getRuleSetPattern create(String lineSeparator) {
		nl = lineSeparator;
		getRuleSetPattern result = new getRuleSetPattern();
		nl = null;
		return result;
	}
	...
	protected final String TEXT_1 = "\t\tRuleSet result = null;" + NL + "\t\tfor (RuleSet set : getRuleSets()) {" + NL + "\t\t\tViewpoint target = set.getTarget();" + NL
			+ "\t\t\tif (target != null && vp.getId().equals(target.getId())) {" + NL + "\t\t\t\tif (result != null)" + NL
			+ "\t\t\t\t\tthrow new IllegalStateException(\"Multple ruleset is not expected\");" + NL + "\t\t\t\tresult = set;" + NL + "\t\t\t}" + NL + "\t\t}" + NL + "\t\treturn result;";

	protected final String TEXT_2 = NL;

	public getRuleSetPattern() {
	...
	public boolean preCondition(PatternContext ctx) throws Exception {
		...
		return "getRuleSet".equals(operation) && "Workspace".equals(cls);
