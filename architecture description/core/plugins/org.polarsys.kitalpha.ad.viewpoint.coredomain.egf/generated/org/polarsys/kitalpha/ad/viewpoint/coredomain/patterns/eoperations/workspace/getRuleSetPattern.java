...
public class getRuleSetPattern
  ...
  protected final String TEXT_1 = "\t\tRuleSet result = null;" + NL + "\t\tfor (RuleSet set : getRuleSets()) {" + NL
      + "\t\t\tViewpoint target = set.getTarget();" + NL
      + "\t\t\tif (target != null && vp.getId().equals(target.getId())) {" + NL + "\t\t\t\tif (result != null)" + NL
      + "\t\t\t\t\tthrow new IllegalStateException(\"Multple ruleset is not expected\");" + NL + "\t\t\t\tresult = set;"
      + NL + "\t\t\t}" + NL + "\t\t}" + NL + "\t\treturn result;";
