...
public class getServiceSetPattern extends org.eclipse.egf.emf.pattern.model.call.Class.ClassimplementedGenOperationTODOoverride {
	protected static String nl;

	public static synchronized getServiceSetPattern create(String lineSeparator) {
		nl = lineSeparator;
		getServiceSetPattern result = new getServiceSetPattern();
		nl = null;
		return result;
	}
	...
	protected final String TEXT_1 = "\t\tServiceSet result = null;" + NL + "\t\tfor (ServiceSet set : getServiceSets()) {" + NL + "\t\t\tViewpoint target = set.getTarget();" + NL
			+ "\t\t\tif (target != null && vp.getId().equals(target.getId())) {" + NL + "\t\t\t\tif (result != null)" + NL
			+ "\t\t\t\t\tthrow new IllegalStateException(\"Multple ServiceSet is not expected\");" + NL + "\t\t\t\tresult = set;" + NL + "\t\t\t}" + NL + "\t\t}" + NL + "\t\treturn result;";

	protected final String TEXT_2 = NL;

	public getServiceSetPattern() {
	...
	public boolean preCondition(PatternContext ctx) throws Exception {
		...
		return "getServiceSet".equals(operation) && "Workspace".equals(cls);
